...
def incl_match(word, incletters):
    '''return true if word contains the include letters'''
    for letter in incletters:
        if letter not in word:
            return False
    return True


def excl_match(word, excletters):
    '''return true if word does not contain the exlude letters'''
    for letter in excletters:
        if letter in word:
            return False
    return True
...
def wordfind(partial_word, incletters, excletters):
    ...
    # filter for words matching partial word
    for word in wordlist[wordlen]:
        if word_match(partial_word, word, wordlen):
            resultlist.append(word)

    # filter for words matching include letters
    if len(incletters) > 0:
        for word in resultlist:
            if incl_match(word, incletters):
                inclist.append(word)
        resultlist = inclist

    # filter for words matching exclude letters
    if len(excletters) > 0:
        for word in resultlist:
